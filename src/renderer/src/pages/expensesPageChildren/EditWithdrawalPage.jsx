...
const EditWithdrawalPage = () => {
  ...
  React.useEffect(() => {
    setSelectedDepartment(cartData?.cart?.departmentName)
    setCartDate(cartData?.cart?.date)
  }, [cartData])
  ...
  return (
    <section className="flex gap-2 p-2 h-full">
        ...
        <ul className="py-4 max-h-[50rem] overflow-y-scroll">
          {cartData &&
            ...
            cartData?.items.map((item, index) => (
              <li
                className={clsx(
                  'text-mainText font-semibold h-10 flex items-center border-b-2 border-mainBlue hover:bg-mainText/15 duration-200 cursor-pointer',
                  {
                ...
                key={item?.id}
              >
                <ul className="flex justify-between w-full max-w-[60rem] text-xs">
...
export default EditWithdrawalPage
