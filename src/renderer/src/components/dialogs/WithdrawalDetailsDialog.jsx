...
import PrintableWithdrawal from '../printableComponents/PrintableWithdrawal'
import { deleteWithdrawal } from '../../util/apis/expensesAPI'
import { useNavigate } from 'react-router'
import { settingCartToEdit } from '../../util/apis/expensesCartApi'

export default function WithdrawalDetailsDialog() {
  const open = useSelector((state) => state.dialog.dialog)
  const receipt = useSelector((state) => state.dialog.withdrawalDetails)
  ...
  const { mutate: setCart } = useMutation({
    mutationFn: settingCartToEdit,
    onSuccess: () => {
      dispatch(dialogActions.handleCloseWithdrawalDetails())
      navigate(`/editWithdrawal?id=${receipt?.id}`)
    }
  })
  ...
  const { mutate: deleteThisReceipt } = useMutation({
        ...
        <DialogContent>
          <section className="px-8 flex flex-col gap-4 items-start relative">
            <div className="hidden">
              <PrintableWithdrawal item={receipt} contentRef={contentRef} />
            </div>
            ...
            {isDeleting && (
              <div className="absolute right-16 top-0 flex flex-col items-center">
                ...
                <div className="flex gap-4 mt-4">
                  <button
                    onClick={() => deleteThisReceipt(receipt?.id)}
